...
from evalorch.core.metrics import JOBS_CREATED, TICKETS_CREATED
from evalorch.models.entities import EvaluationJob, EvaluationTicket, JobProgress
from evalorch.models.enums import CheckType, JobStatus, TicketStatus
...
from evalorch.services.dataset_folder import (
    DatasetFolderError,
    PayloadFile,
    read_dataset_folder,
)
...
def _result_for_existing_job(
    ...
    # Snapshot key "agents": agent id to the frozen metrics for that agent.
    agents_by_id = snapshot.get("agents") or {}
    # API field profiles: the agent ids that selected those metrics.
    profiles = list(agents_by_id)
    metric_ids: list[str] = []
    for agent_doc in agents_by_id.values():
    ...
    for payload in snapshot.get("payloads") or []:
        ...
        metrics = (agents_by_id.get(agent_id) or {}).get("metrics") or []
        if agent_id:
...
def create_job(
    ...
    *,
    ...
    if key is not None and len(key) > settings.idempotency_key_max_length:
    ...
    # Agent id to the frozen metrics selected for that agent.
    metrics_by_agent: dict[str, list[dict[str, Any]]] = {}
    prepared: list[tuple[PayloadFile, uuid.UUID, str, list[dict[str, Any]]]] = []
    warnings = list(folder.warnings)
    metric_ids: list[str] = []
    # API field profiles: agent ids returned with the created job.
    profiles: list[str] = []
    ...
    for payload_file in folder.payloads:
        ...
        try:
            ...
            if agent_id not in metrics_by_agent:
                metrics_by_agent[agent_id] = metrics_for_agent(folder.config, agent_id, source=source)
                profiles.append(agent_id)
            snapshots = metrics_by_agent[agent_id]
