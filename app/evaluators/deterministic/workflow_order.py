...
SPAN_IDENTITY_KEYS = ("agent_id", "agent_type", "span_id", "name")
...
def _span_matches(span: dict[str, Any], wanted: str) -> bool:
    return any(str(span.get(key, "")) == wanted for key in SPAN_IDENTITY_KEYS)
...
class WorkflowOrderEvaluator(Evaluator):
    ...
    def evaluate(
        ...
        present = [name for name in expected_names if any(_span_matches(s, name) for s in spans)]
...
class SpanExistsEvaluator(Evaluator):
    ...
    def evaluate(
        ...
        found = [name for name in wanted if any(_span_matches(s, name) for s in spans)]
